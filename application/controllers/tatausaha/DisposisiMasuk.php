...
class DisposisiMasuk extends CI_Controller {
	...
	function index($p=0){
		...
		$recordperpage = 10;
		$mypaging['base_url'] = base_url().'tatausaha/disposisimasuk/index/';
		if($user['rank']=='Admin Sekda') {
			$mypaging['total_rows'] = $this->TataUsahaModels->query("SELECT * FROM t_suratkeluar where teruskan='Sekretaris Daerah'")->num_rows();
		} else if($user['rank']=='Admin Ass 1') {
			$mypaging['total_rows'] = $this->TataUsahaModels->query("SELECT * FROM t_suratkeluar where teruskan='Asisten Pemerintah dan Kesra'")->num_rows();
		} else if($user['rank']=='Admin Ass 2') {
			$mypaging['total_rows'] = $this->TataUsahaModels->query("SELECT * FROM t_suratkeluar where teruskan='ASISTEN PEREKONOMIAN DAN PEMBANGUNAN'")->num_rows();
		} else if($user['rank']=='Admin Ass 3') {
			$mypaging['total_rows'] = $this->TataUsahaModels->query("SELECT * FROM t_suratkeluar where teruskan='ASISTEN ADMINISTRASI UMUM'")->num_rows();
		} else {
			$mypaging['total_rows'] = $this->TataUsahaModels->query("SELECT * FROM t_suratkeluar where teruskan='KABAG. UMUM'")->num_rows();
		}
		...
		$user = $this->session->userdata['logged_in'];
		if($user['rank']=='Admin Sekda') {
			$data['suratmasukuser'] =  $this->UserModels->listingsurat($recordperpage,$p,'t_suratkeluar','tgl_keluar','ASC','status','Belum Dibaca','teruskan','Sekretaris Daerah');
			...
			$data['suratmasukuser'] =  $this->UserModels->listingsurat($recordperpage,$p,'t_suratkeluar','tgl_keluar','ASC','status','Belum Dibaca','teruskan','Asisten Pemerintah dan Kesra');
			...
			$data['suratmasukuser'] =  $this->UserModels->listingsurat($recordperpage,$p,'t_suratkeluar','tgl_keluar','ASC','status','Belum Dibaca','teruskan','ASISTEN PEREKONOMIAN DAN PEMBANGUNAN');
			...
			$data['suratmasukuser'] =  $this->UserModels->listingsurat($recordperpage,$p,'t_suratkeluar','tgl_keluar','ASC','status','Belum Dibaca','teruskan','ASISTEN ADMINISTRASI UMUM');
			...
			$data['suratmasukuser'] =  $this->UserModels->listingsurat($recordperpage,$p,'t_suratkeluar','tgl_keluar','ASC','status','Belum Dibaca','teruskan','KABAG UMUM');
		...
		$this->load->view('tatausaha/dismasuk',$data);
	}
	...
	function detailkeluar($id){
		...
		$data['logged_in'] = $this->session->userdata('logged_in');
		$data['kantor'] = $this->UserModels->query("SELECT * FROM tr_instansi LIMIT 1")->row();
		$this->load->view('tatausaha/detailsuratlain',$data);
	}
}