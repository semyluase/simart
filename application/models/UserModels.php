...
class UserModels extends CI_Model {
	...
	function listing($recordperpage,$page,$tbl_name,$order_by,$asc_desc,$col_name,$where){
		$this->db->select('*');
	...
	function listingsurat($recordperpage,$page,$tbl_name,$order_by,$asc_desc,$col_name,$where,$col_name1,$where1){
		$this->db->select('*');
		$this->db->from($tbl_name);
		$this->db->order_by($order_by,$asc_desc);
		$this->db->where($col_name,$where);
	...
	function editRecordLain($col_name1,$where1,$col_name,$where,$tbl_name,$data){
		$this->db->where($col_name,$where);
		$this->db->where($col_name1,$where1);
		$this->db->update($tbl_name,$data);
	}
